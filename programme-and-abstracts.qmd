...
**8:00-8:20** Self-registration: pick up your name badge

**8:20-8:30** Opening session

**8:30-9:30** Keynote talk 1 by Finn Lindgren (University of Edinburgh): *Markovian potatoes - a collaborative effort*. <span style="color:grey">**Chair**: Sara Martino.</span>

**9:30-10:30** Invited talks. <span style="color:grey">**Chair**: Virgilio Gómez-Rubio.</span>
...
**11:00-12:30** Contributed talks. <span style="color:grey">**Chair**: Amanda Lenzi.</span>
...
**8:30-9:30** Keynote talk 2 by Sara Martino (NTNU): 20 years of INLA. <span style="color:grey">**Chair**: Andrea Riebler.</span>

**9:30-10:30** Panel discussion: *Implementing INLA - Past, Present and Future*. <span style="color:grey">**Chair**: Lisa B Gaedke-Merzhäuser.</span>

**10:30-10:55** Coffee break

**11:00-12:00** Invited talks. <span style="color:grey">**Chair**: Geir-Arne Fuglstad.</span>
...
**15:30- 15:55** Coffee break & photo

**16:00-17:00** Keynote talk 3 by Janet van Niekerk (KAUST): *INLA 2.0*. <span style="color:grey">**Chair**: David Bolin.</span>
...
**8:30-9:30** Keynote talk 4 by Elias Teixeira Krainski (KAUST): *graphpcor: Models for correlation matrices based on graphs*. <span style="color:grey">**Chair**: Sigrunn Holbek	Sørbye.</span>

**9:30-11:00** Contributed talks. <span style="color:grey">**Chair**: Janet van Niekerk.</span>
...
- **10:24-10:42** Luisa	Ferrari	(University of Modena and Reggio Emilia) *A Standardization Procedure to Incorporate Variance Partitioning-Based Priors in Latent Gaussian Models*
- **10:42-11:00** Birgir Hrafnkelsson (University of Iceland) *Max-and-Smooth: A Two-Step Approach for Approximate Bayesian Inference in Latent Gaussian Models*
- **11:00-11:18** Brynjólfur Gauti Guðrúnar Jónsson (University of Iceland) *Modeling spatial dependence through latent Gaussian models with spatial copulas*

**11:20-11:55** Coffee break

**12:00-12:30** Invited talk by Dan Simpson (.txt). <span style="color:grey">**Chair**: Sara Martino.</span>

**12:30-13:15** Awards, Håvard's speech, and closing
...
Please [click here](https://www.dropbox.com/scl/fi/1jmem17egthm6jpo35qpy/book-of-abstracts-pdf.pdf?rlkey=a6z4ebyksv6js5x8udpjainwa&dl=1) to download a pdf version.
...
::: {.callout-note collapse="true" appearance="simple" icon=false}
## Vincent	Maillou	(ETH Zurich): A Novel Framework to Perform Large Scale Bayesian Modeling
Bayesian inference on large-scale spatio-temporal models is limited by its computational feasibility, a trend that is further exacerbated by the continuous increase in data availability and model refinements. To address this issue, we present a novel standalone Python implementation of the methodology of integrated nested Laplace approximations (INLA). Our framework aims at combining modern coding practices, code modularity and high-performance computing capabilities. It achieves unprecedented scalability through GPU-acceleration and a triple-layer, nested, distributed-memory parallelization strategy. We showcase the computational performance of our framework on up to 288 Hopper GPUs of the CSCS Alps supercomputer, scaling further a large spatio-temporal air temperature prediction model made of a spatial mesh composed of 4002 edges over the course of 250 days. Our broader goal being to extend the scope of our novel framework to support other large-scale models that exceed the computational limits of the R-INLA package.
...
::: {.callout-note collapse="true" appearance="simple" icon=false}
## Kelvin Jhonson Ribeiro de Sousa Almeida Silva (KAUST): A class of Finite Volume Method-based Numerical Approximation for Fractional Stochastic Partial Differential Equations driven by non-Gaussian White Noises
The so-called SPDE approach for Gaussian fields is a computationally efficient method widely used in spatial and spatio-temporal statistics. This methodology involves proposing latent models whose structures approximate the solutions of stochastic partial differential equations (SPDEs) driven by Gaussian white noise. The primary technique for these approximations is the well-established Finite Element Method (FEM). To enhance computational efficiency, practical implementations rely on the lumped mass technique (LM), which leads to the use of lumped mass FEM (LM-FEM) discretisations. This creates a divergence between theory and practice, as numerical analyses are typically performed on standard FEM discretisations, whereas real-world applications implement the LM-FEM variant. Furthermore, the increasing demand for robust models based on non-Gaussian latent structures necessitates a deeper understanding of the degrees of freedom in selecting the driving noise. It also calls for the development of alternative methods to handle non-Gaussian driving terms, given that the FEM framework becomes impractical in these contexts. In this work, we establish the foundational theory for employing the Finite Volume Method (FVM) as an alternative to discretise SPDEs driven by a broad class of white noises—including all square integrable Lévy white noises. We also provide the rigorous theoretical foundation to introduce the LM-FVM, which had previously been misidentified as LM-FEM. The key advantage of this approach is the unified framework that aligns theoretical analyses with practical implementations. Notably, our numerical analysis of the covariance functions associated with discrete non-Gaussian random fields demonstrates that the LM-FVM discretisations converge at the same rate as their FEM counterparts used for Gaussian white noises, while being computationally more efficient.
:::
...
::: {.callout-note collapse="true" appearance="simple" icon=false}
## Dionisio	Silva Neto (KAUST): A new approach for joint modeling with cure fraction using Gompertz defective distribution
Joint models for longitudinal and survival data have gained significant importance in clinical trials due to their predictive and inferential advantages in risk processes associated with biomarkers. Within the literature, few applications are designed for experiments involving a cure fraction, in which a portion of the individuals under observation will never experience the event of interest. The aim of this work is to present a joint model based on the framework of defective distributions, which offer a more integrated and parsimonious approach compared to modeling under the standard risk model. We formulate the joint model within a Bayesian framework, assuming the Gompertz model as the failure time specification. For parametric estimation, we used the efficient Integrated Nested Laplace Approximation (INLA) method. Under vague prior specifications, we conducted simulation studies to assess some frequentist properties under different censoring schemes. Finally, we applied the proposed methodology to an experiment involving three discrete biomarkers in the context of treatment failure among patients with a documented history of two or more clinically definite unprovoked epileptic seizures.
:::
...
::: {.callout-warning collapse="true" appearance="simple" icon=false}
## Keynote talk 3: Janet van Niekerk
...
::: {.callout-warning collapse="true" appearance="simple" icon=false}
## Keynote talk 4: Elias Teixeira Krainski
...
**Title**: All Hail the Markov Property

**Summary.**
All Hail the Markov Property; or Heaping praise on a very dead Russian in aid of celebrating a very alive Norwegian for hopefully 30 mins because some of you have planes and trains to catch
